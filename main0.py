...
def main(main_data, bot_data):
    ...
    can_data_new = scan("CAN", main_data["CAN"], bot_data)
    ...
    data_new = {"TF": tf_data_new, "ST": st_data_new, "PG": pg_data_new, "PAR": par_data_new, "SA3": sa3_data_new,
                "CST": cst_data_new, "ZG": zg_data_new,"CAN": can_data_new}
...
def salva_allarmi(data):
    ...
    stato_allarmi = {
        ...
        "ZG": data["ZG"]["Plant state"], "CAN": data["CAN"]["Plant state"]
    }
...
CANData = {"Plant state": CANState}

cycleN = 1
Data = {"TF": TFData, "ST": STData, "PG": PGData, "PAR": PARData, "SA3": SA3Data, "CST": CSTData, "ZG": ZGData, "CAN": CANData}
...
def send_last24_resa(mode):
    ...
    # ---CAN--- #
    filename = "CANlast24hStat.csv"
    df_pg = pd.read_csv(filename)
    energy_can = df_pg["Energy"][0]

    rating_dict = {"Nome": ["Torrino Foresta", "San Teodoro", "Partitore", "Ponte Giurino", "Canaletta"],
                   "Energia": [energy_tf, energy_st, energy_par, energy_pg, energy_can]}
    ...
    energy4, dummy = convertNumber(df["Energia"][4], "Energy", "HTML", "CAN")
...
def save_portale_impianti_hp():
    ...
    can_data = pd.read_csv("CAN_dati_gauge.csv")
    ...
    can_day = pd.read_csv("CANDayStat.csv")
    ...
    energy_can_day = can_day["Energy"][0]
    print("Energia canaletta: ", energy_can_day)
    ...
    can_row = {"TAG": "CAN", "Name": "Canaletta", "last_power": can_power, "state": can_alarm,
              "Energy": energy_can_day}
    ...
    can_df = pd.DataFrame(can_row, index=[0])

    df = pd.concat([st_df, par_df, tf_df, pg_df, zg_df, can_df])
